...
module "s3-website" {
  source                  = "modules/terraform-s3-website"
  bucket_name             = join("-", [var.env, var.account_name])
  index_file_contents_b64 = module.award.content_b64
  error_file_contents_b64 = module.award.content_b64
}
